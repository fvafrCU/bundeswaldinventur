...
if (interactive()) {
  ...
  require("bundeswaldinventur", quietly = TRUE, character.only = TRUE) ||
  ...
  if (any(grepl("--file", r_call))) {
    ...
    unit_dir <- system.file("runit_tests", package = "bundeswaldinventur")
  }
}
...
package_suite <- RUnit::defineTestSuite("bundeswaldinventur_unit_test",
  dirs = unit_dir,
  testFileRegexp = "^.*\\.[rR]",
