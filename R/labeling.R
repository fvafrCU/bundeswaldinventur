...
map_labels_to_abbreviations <- function(x) {
  if (is.factor(x)) {
    levels(x) <- vapply(
      levels(x), bundeswaldinventur::get_abbreviation_for_label,
      character(length = 1)
    )
  } else {
    x <- vapply(
      as.character(x), bundeswaldinventur::get_abbreviation_for_label,
      character(length = 1)
    )
  }
  return(x)
...
map_abbreviations_to_labels <- function(x) {
  if (is.factor(x)) {
    levels(x) <- vapply(
      levels(x), get_text_label_for_abbreviation,
      character(length = 1)
    )
  } else {
    x <- vapply(
      as.character(x), get_text_label_for_abbreviation,
      character(length = 1)
    )
  }
  return(x)
...
get_colors_for_ownership <- function(x) {
  y <- vapply(
    as.character(x), get_color_for_ownership,
    character(length = 1)
  )
  return(y)
}
