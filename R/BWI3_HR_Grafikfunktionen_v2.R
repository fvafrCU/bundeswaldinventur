...
#k\u00e4/25.09.13
{
  ...
  graphics::plot(x=1:k,Y,"n",lwd=20,col="grey70",xaxt="n",
  ...
  for (i in 1:k)
  {
    ...
    y1 <- Y[i]+se.Y[i]
    x0 <- i; x1 <- i
    graphics::segments(x0,y0,x1,y1,col="grey",lwd=2)
    x0 <- i-0.1; x1 <- i+0.1;
    graphics::segments(x0,y0,x1,y0,col="grey",lwd=2)
    graphics::segments(x0,y1,x1,y1,col="grey",lwd=2)
  }
}
#-------------------------------------------------------------------------------
...
#<bar.lab> enth\u00e4lt die Bezeichnungen je Datenreihe
{
  ...
  se.Y[is.nan(se.Y)]  <- 0
  p <- 5*10^(floor(log(max(Y)+max(se.Y))/log(10))-1)
  graphics::plot(x=1:k,Y[,1],"n",lwd=20,col="grey70",xaxt="n",
        xlim=c(dx,k+dx),ylim=c(0,ceiling((max(Y)+max(se.Y))/p)*p),
        main=titel,xlab="Baumartengruppe", ylab=y.lab,
        sub=Sys.time(),cex.sub=0.5)
  graphics::axis(side=1,at=1:k,labels=bagr.list)
...
#f\u00fcr eine klassifizierte Zielgr\u00f6\u00dfe (AKl oder DKl) k\u00e4/30.09.13
{
  ...
  #Legende
  graphics::legend("topleft",legend=bar.lab,fill=bar.clr)
...
#f\u00fcr eine klassifizierte Zielgr\u00f6\u00dfe (AKl oder DKl) k\u00e4/16.03.2014
{
  ...
  for (i in 1:k)
  {
    ...
    if (!is.na(se.Y[i]))
    {
      ...
      y1 <- Y[i,r]+se.Y[i,r]
      x0 <- i; x1 <- x0
      graphics::segments(x0,y0,x1,y1,col="grey10",lwd=2)
      x0 <- x0-0.1; x1 <- x1+0.1;
      graphics::segments(x0,y0,x1,y0,col="grey30",lwd=2)
      graphics::segments(x0,y1,x1,y1,col="grey30",lwd=2)
    }
  }
  ...
  r <- 1
  graphics::lines(1:k,Y[,r],col=line.clr,lwd=l.wd)
  #Fehlerbalken
  for (i in 1:k)
  {
    if (!is.na(se.Y[i,r]))
    {
      ...
      y1 <- Y[i,r]+se.Y[i,r]
      x0 <- i; x1 <- x0
      graphics::segments(x0,y0,x1,y1,col=line.clr,lwd=2)
      x0 <- x0-0.1; x1 <- x1+0.1;
      graphics::segments(x0,y0,x1,y0,col=line.clr,lwd=2)
      graphics::segments(x0,y1,x1,y1,col=line.clr,lwd=2)
    }
  }

  #Legende
  graphics::legend("topright",legend=bar.lab,fill=c(line.clr,bar.clr))
...
#k\u00e4/16.03.2014
{
  ...
  for (r in 1:n.r)
  {
    graphics::lines(1:k,Y[,r],col=line.clr[r],lwd=l.wd[r])
    #Fehlerbalken
    for (i in 1:k)
    {
      if (!is.na(se.Y[i,r]))
      {
        ...
        y1 <- Y[i,r]+se.Y[i,r]
        x0 <- i; x1 <- x0
        graphics::segments(x0,y0,x1,y1,col=line.clr[r],lwd=2)
        x0 <- x0-0.1; x1 <- x1+0.1;
        graphics::segments(x0,y0,x1,y0,col=line.clr[r],lwd=2)
        graphics::segments(x0,y1,x1,y1,col=line.clr[r],lwd=2)
      }
    }
  }

  #Legende
  graphics::legend("topright",legend=line.lab,lwd=l.wd,col=line.clr)
...
#k\u00e4/16.03.2014
{
  ...
  for (r in 1:n.r)
  {
    graphics::lines(1:k,Y[,r],col=line.clr[r],lwd=l.wd[r])
    #Fehlerlinien
    graphics::lines(1:k,(Y[,r]-se.Y[,r]),col=line.clr[r],lwd=1,lty=2)
    graphics::lines(1:k,(Y[,r]+se.Y[,r]),col=line.clr[r],lwd=1,lty=2)
  }
...
#<kat.col>: Farben f\u00fcr die Kategorien innerhalb der gestapelten S\u00e4ulen
{
 ...
 for (j in 1:length(y.mat[1,]))
 {
  ...
  for (i in 1:length(y.mat[,1]))
  {
    ...
    y <- y + y.mat[i,j]
    graphics::text(p[j],y-y.mat[i,j]/2+0.03*y.max,paste(y.mat[i,j]),pos=1)
  }
 }
...
if(F)
{
 ...
 for (j in 1:length(y.mat[1,]))
 {
  ...
  for (i in 1:length(y.mat[,1]))
  {
    #print(paste(y.mat[i,j]))
    y <- y + y.mat[i,j]
    graphics::text(p[j],y-y.mat[i,j]/2+3,paste(y.mat[i,j]),pos=1)
  }
 }
 points(1,10,pch=20)
