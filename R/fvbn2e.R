...
#'  (vektor mit berechneten pro ha Attributname), \strong{Groessen} (Vektor mit
...
FVBN.bagrupp.akl.dkl.stratum.fun.2e <-
  function(baeume, ecken, trakte, A, inv, BA.grupp, A.klass, D.klass, auswahl) {
        ...
        for (k in 1:D.k) # Durchmesserklassen
        {
          if (j == A.k) # alle AKl
          {
            if (k == D.k) {
              ...
              baeume.ba <- baeume.s[
                baeume.s[["bagr"]] == bagr.list[i] & baeume.s[["dkl"]] == dkl.lab[k],
                c("tnr", "enr", "bhd", "dkl", "volv", "oib", "nha", "stfl")
              ]
            }
          } else {
            if (k == D.k) {
              baeume.ba <- baeume.s[
                baeume.s[["bagr"]] == bagr.list[i] & baeume.s[["akl"]] == akl.lab[j],
                c("tnr", "enr", "bhd", "dkl", "volv", "oib", "nha", "stfl")
              ]
            } else {
              baeume.ba <- baeume.s[
    ...
    return(list(
      ...
      Attribute2 = c(
        "BA_Proz", "V_DhmR/ha", "V_DhmR_HB/ha", "oiB/ha", "N/ha",
        "N_Dh/ha", "N_Dh_HB/ha"
      ),
