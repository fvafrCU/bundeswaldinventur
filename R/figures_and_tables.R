...
theme_bold_legend <- function() {
  return(
    ggplot2::theme(
      legend.title = ggplot2::element_text(colour = "black", size = 16),
...
melt_species_attribute_group <- function(list, species, abbreviation, group) {
  ...
  bwi1_statistics <- data.frame(
    prediction = get(
      array_containing_statistics,
      bwi1
    )[
      attribute_index,
      1, species_index,
    ...
    standard_error = get(array_containing_statistics, bwi1)[
      attribute_index,
      2, species_index,
      ,
    ],
  ...
  bwi2_statistics <- data.frame(
    prediction = get(array_containing_statistics, bwi2)[
      attribute_index,
      1, species_index,
      ,
    ],
    standard_error = get(array_containing_statistics, bwi2)[
      attribute_index,
      2, species_index,
      ,
    ],
  ...
  bwi3_statistics <- data.frame(
    prediction = get(array_containing_statistics, bwi3)[
      attribute_index,
      1, species_index,
      ,
    ],
    standard_error = get(array_containing_statistics, bwi3)[
      attribute_index,
      2, species_index,
      ,
    ],
...
plot_species_attribute_group <- function(data) {
  return(
    ggplot2::ggplot(
      ...
      ggplot2::xlab(get_label_for_abbreviation(
        as.character(unique(data$grouping_variable)),
        "text_label"
      )) +
      ggplot2::scale_y_continuous(labels = scales::comma) +
      ggplot2::ylab(get_label_for_abbreviation(
        as.character(unique(data$abbreviation)),
        "axis_label"
      )) +
      ggplot2::ggtitle(paste(
        "Baumartengruppe", unique(data$species), "im",
