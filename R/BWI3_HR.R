          ...
          for (i.n in 1:n.nart) # Nutzungsart (geerntet, ungenutzt), wenn gesetzt!
          {
            ...
            if (i < n.bagr) {
              baeume.ba <- baeume.s[
                ...
                c(
                  "tnr", "enr", "pk", "volv2", "vole2", "oib2", "nha1", "stfl1",
                  "plkal"
                )
              ]
            } else {
...
iVB.ew.bagrupp.akl.dkl.stratum.fun.2 <- function(baeume.23, baeume.3,
  ...
  baeume.3.s <- merge(baeume.3[TRUE, c(
    "TNr", "ENr", "STP", "BNr", "Entf", "Pk",
  ...
  baeume.2.s <- merge(baeume.23[TRUE, c(
    "TNr", "ENr", "STP", "BNr", "Entf",
  ...
  # Zur Kontrolle
  sum(baeume.3.s$StFl2) / 10000
  sum(baeume.2.s$StFl1) / 10000
  ...
  iv.es.a.t.bagr.akl.dkl <- iVB.bilanz.bagr.akl.dkl.fun(
    baeume.2.s, baeume.3.s,
    ecken
  )
  ...
  iv.es.a.t.alle.akl.dkl <- iVB.bilanz.bagr.akl.dkl.fun(
    baeume.2.s, baeume.3.s,
    ecken
  )
  #++++
  ...
  for (i in 1:n.bagr)
  {
    for (j in 1:A.k)
    {
      for (k in 1:D.k)
      {
        ...
        #-----------------------------------------------------------------------
        iv.bil.t <- iv.es.a.t[TRUE, c(
          "TNr", "mBAF", "mBAF.oLK", "mPL", "mPLkal",
  ...
  return(list(
    Log = Log, Stratum = auswahl,
    HBF = HBF, se_HBF = se.HBF,
    Attribute = c(
      "V_DhmR", "V_DhmR_HB", "V_EoR", "oiB",
...
iVB.ew.bagrupp.akl.dkl.stratum.fun.2g <- function(baeume.23, baeume.3,
  ...
  # Bilanzierungs-Komponenten klassifiziert nach BaGr, akl und dkl
  iv.es.a.t.bagr.akl.dkl <- iVB.bilanz.bagr.akl.dkl.fun.2g(
    baeume.2.s, baeume.3.s,
    ecken
  )
  #++++
  ...
  baeume.2.s$BaGr <- "AlleBA"
  baeume.3.s$BaGr <- "AlleBA"
  iv.es.a.t.alle.akl.dkl <- iVB.bilanz.bagr.akl.dkl.fun.2g(
    baeume.2.s, baeume.3.s,
    ecken
  )
  #++++
...
verbiss.bagr.fun <- function(verj, ecken, trakte, auswahl, inv, A,
  ...
  for (i in 1:n.bagr)
  {
    ...
    for (j in 1:4)
    {
      vb.bagr <- r.variance.fun(
        cbind(verbiss.t.bagr$t, verbiss.t.bagr[, (3 + j)]),
        length(trakte[, 1])
      )
  ...
  for (i in 1:4)
  {
    r.list <- r.variance.fun(
      cbind(verbiss.t$t, verbiss.t[, (2 + i)]),
      length(trakte[, 1])
    )
  ...
  return(list(
    Log = Log, Stratum = auswahl, HBF = T.hbf, se.HBF = se.T.hbf,
    BAGR = bagr.list,
...
verjg.kl4.bagr.fun <- function(verj.kl4, ecken, trakte, auswahl, A,
  ...
  verj.kl4.s <- merge(verj.kl4.s, bacode[TRUE, c("ICode", "BaGr")],
    by.x = "ba",
    by.y = "ICode", all.x = T
  )
  ...
  for (i in 1:n.bagr)
  {
    # BAGR
      ...
      if (identical(length(idx), 1L) && is.na(idx)) idx <- NULL
      vj.kl4.t.bagr <- vj.kl4.t[idx, TRUE]
    # Nach Verj\u00fcngungsarten
    for (j in 1:n.vart)
    {
      vj.kl4.t.vart <- vj.kl4.t.bagr[
        vj.kl4.t.bagr[["vart"]] == vart.list[j],
        c("tnr", "baf")
      ]
...
verjg.kl4.bagrupp.fun <- function(verj.kl4, ecken, trakte, ba.grupp, auswahl, A) {
  ...
  vj.kl4.t <- tryCatch(stats::aggregate(verj.kl4.s$baf,
    by = list(
      verj.kl4.s$tnr, verj.kl4.s$bagr,
      verj.kl4.s$vjgart
    ),
    sum
  ),
...
biotop.baeume.fun <- function(wzp4.merkmale, baeume, ecken, trakte, A, auswahl) {
  ...
  wzp4.merkmale <- merge(baeume[
    baeume$stp == 0,
    c("tnr", "enr", "bnr", "ba", "alt", "bhd", "h", "volv", "oib", "nha", "stfl")
  ],
  wzp4.merkmale,
  by = c("tnr", "enr", "bnr"), all.x = T
  )
  ...
  wzp4.merkmale.s <- merge(wzp4.merkmale[TRUE, c(
    "tnr", "enr", "bnr", "pk",
    "biotopb", "ba", "alt", "bhd",
    "volv", "oib", "nha", "stfl"
  )],
  ...
  names(biotop.t) <- c(
    "tnr", "N.BB", "V.BB", "oiB.BB", "SthV.BB", "V.BB.LB",
    "V.BB.NB"
  )
  utils::head(biotop.t)
  ...
  return(list(
    ...
    Biotopbaumkennwerte =
      data.frame(
        ...
        Wert = c(
          T.N.BB, N.BB.ha, T.V.BB, V.BB.ha, V.BB.Stck, T.V.BB.LB,
          V.BB.LB.ha, T.V.BB.NB, V.BB.NB.ha, T.oiB.BB, oiB.BB.ha
        ),
        SE = c(
          se.T.N.BB, se.N.BB.ha, se.T.V.BB, se.V.BB.ha, se.V.BB.Stck,
...
stamm.merkmale.bagr.fun <- function(wzp4.merkmale, baeume, ecken, trakte, A,
  ...
  for (i in 1:n.bagr)
  {
    ...
    merkmal.t <- merge(xy[TRUE, c("tnr", "m", "hbf")], merkmal.t,
      by = "tnr",
      all.x = T
    )
    ...
    attr.bagr.tab[, 1, i] <-
      c(
        T.N.MM, N.MM.ha, T.V.MM, V.MM.ha, V.MM.Stck, T.oiB.MM, oiB.MM.ha,
        Anteil.MM.N * 100, Anteil.MM.V * 100
      )
    attr.bagr.tab[, 2, i] <- c(
      se.T.N.MM, se.N.MM.ha, se.T.V.MM, se.V.MM.ha, se.V.MM.Stck,
...
mbaf.bagr.alt.bhd.pm.fun <- function(baeume.vor, baeume.folg, A.klass, D.klass) {
  ...
  baf12.tnr <- merge(baf1.tnr, baf2.tnr,
    by = c("tnr", "bagr", "akl.pm", "dkl.pm"),
    all.x = T, all.y = T
  )
  # NA entfernen
...
iVB.bilanz.bagr.akl.dkl.fun <- function(baeume.vor, baeume.folg, ecken) {
  ...
  baeume.ba <- baeume.folg[TRUE, c(
    "TNr", "ENr", "STP", "BNr", "Pk", "BHD1",
  ...
  names(iv.es.t) <- c(
    "TNr", "BaGr", "Akl", "Dkl", "iV.DhmR.S", "iV.DhmR.HB.S",
  ...
  names(iv.a.t) <- c(
    "TNr", "BaGr", "Akl", "Dkl", "iV.DhmR.A", "iV.DhmR.HB.A",
...
akl.lab.fun <- function(A.klass, A.k) {
  ...
  if (A.k > 1) {
    ...
    akl.lab[A.k] <- paste("0-", A.klass[[1]], sep = "")
  }
