...
flaechen.kreis.fun.1 <- function(kreiscode,
  ...
  (wfl.stw.3 <-
    fl.stratum.fun(
      list(Wa = c(3, 4, 5), Kreis = krs.list[i], EigArt = c("BW", "StW")),
      ecken.3, trakte.3, A
    ))
  (wfl.stw.2 <-
    fl.stratum.fun(
      list(Wa = c(1:4), Kreis = krs.list[i], EigArt = c("BW", "StW")),
  ...
  (wfl.bw.3 <-
    fl.stratum.fun(
      list(Wa = c(3, 4, 5), Kreis = krs.list[i], EigArt = "BW"),
      ecken.3, trakte.3, A
    ))
  # KW
  (wfl.kw.3 <-
    fl.stratum.fun(
      list(Wa = c(3, 4, 5), Kreis = krs.list[i], EigArt = "KW"),
      ecken.3, trakte.3, A
    ))
  (wfl.kw.2 <-
    fl.stratum.fun(
      list(Wa = c(1:4), Kreis = krs.list[i], EigArt = "KW"),
      ecken.2, trakte.2, A.12
    ))
  (wfl.kw.1 <-
    fl.stratum.fun(
  ...
  (wfl.oew.3 <-
    fl.stratum.fun(list(
      Wa = c(3, 4, 5), Kreis = krs.list[i],
      EigArt = c("BW", "StW", "KW")
    ), ecken.3, trakte.3, A))
  (wfl.oew.2 <-
    fl.stratum.fun(list(
      Wa = c(1:4), Kreis = krs.list[i],
      EigArt = c("BW", "StW", "KW")
    ), ecken.2, trakte.2, A.12))
  (wfl.oew.1 <-
    fl.stratum.fun(list(
  ...
  (wfl.pw.3 <-
    fl.stratum.fun(
      list(Wa = c(3, 4, 5), Kreis = krs.list[i], EigArt = "PW"),
      ecken.3, trakte.3, A
    ))
  (wfl.pw.2 <-
    fl.stratum.fun(
      list(Wa = c(1:4), Kreis = krs.list[i], EigArt = "PW"),
      ecken.2, trakte.2, A.12
    ))
  (wfl.pw.1 <-
    fl.stratum.fun(
  ...
  (wfl.gpw.3 <-
    fl.stratum.fun(
      list(Wa = c(3, 4, 5), Kreis = krs.list[i], EigArt2 = "GPW"),
      ecken.3, trakte.3, A
    ))
  (wfl.gpw.2 <-
    fl.stratum.fun(
      list(Wa = c(1:4), Kreis = krs.list[i], EigArt2 = "GPW"),
      ecken.2, trakte.2, A.12
    ))
  (wfl.gpw.1 <-
    fl.stratum.fun(
  ...
  (wfl.mpw.3 <-
    fl.stratum.fun(
      list(Wa = c(3, 4, 5), Kreis = krs.list[i], EigArt2 = "MPW"),
      ecken.3, trakte.3, A
    ))
  (wfl.mpw.2 <-
    fl.stratum.fun(
      list(Wa = c(1:4), Kreis = krs.list[i], EigArt2 = "MPW"),
      ecken.2, trakte.2, A.12
    ))
  (wfl.mpw.1 <-
    fl.stratum.fun(
  ...
  (wfl.kpw.3 <-
    fl.stratum.fun(
      list(Wa = c(3, 4, 5), Kreis = krs.list[i], EigArt2 = "KPW"),
      ecken.3, trakte.3, A
    ))
  (wfl.kpw.2 <-
    fl.stratum.fun(
      list(Wa = c(1:4), Kreis = krs.list[i], EigArt2 = "KPW"),
      ecken.2, trakte.2, A.12
    ))
  (wfl.kpw.1 <-
    fl.stratum.fun(
  ...
  fl.ant.stw.3 <-
    fl.proz.stratum.fun(
      list(Wa = c(3:5), Kreis = krs.list[i]), list(EigArt = c("BW", "StW")),
      ecken.3
    )
  # K\u00f6rperschaftswald
  fl.ant.kw.3 <-
  ...
  fl.ant.oew.3 <-
    fl.proz.stratum.fun(
      list(Wa = c(3:5), Kreis = krs.list[i]),
  ...
  (bhbf.3 <- fl.stratum.fun(
    list(Wa = c(3, 5), Begehbar = 1, Kreis = krs.list[i]),
    ecken.3, trakte.3, A
  ))
  (bhbf.stw.3 <- fl.stratum.fun(list(
    Wa = c(3, 5), Begehbar = 1, Kreis = krs.list[i],
    EigArt = "StW"
  ), ecken.3, trakte.3, A))
  (bhbf.kw.3 <- fl.stratum.fun(list(
    Wa = c(3, 5), Begehbar = 1, Kreis = krs.list[i],
    EigArt = "KW"
  ), ecken.3, trakte.3, A))
  (bhbf.oew.3 <- fl.stratum.fun(list(
    Wa = c(3, 5), Begehbar = 1, Kreis = krs.list[i],
    EigArt = c("BW", "StW", "KW")
  ), ecken.3, trakte.3, A))
  (bhbf.pw.3 <- fl.stratum.fun(list(
    Wa = c(3, 5), Begehbar = 1, Kreis = krs.list[i],
    EigArt = "PW"
  ), ecken.3, trakte.3, A))
  (bhbf.gpw.3 <- fl.stratum.fun(list(
    Wa = c(3, 5), Begehbar = 1, Kreis = krs.list[i],
    EigArt2 = "GPW"
  ), ecken.3, trakte.3, A))
  (bhbf.mpw.3 <- fl.stratum.fun(list(
    Wa = c(3, 5), Begehbar = 1, Kreis = krs.list[i],
    EigArt2 = "MPW"
  ), ecken.3, trakte.3, A))
  (bhbf.kpw.3 <- fl.stratum.fun(list(
    Wa = c(3, 5), Begehbar = 1, Kreis = krs.list[i],
    EigArt2 = "KPW"
  ), ecken.3, trakte.3, A))
  ...
  return(list(
    ...
    Waldanteil = data.frame(
      ...
      Waldanteil = round(c(
        wfl.3$Flaeche / kreise[kreise$codeKreis == kreiscode, 5],
        wfl.2$Flaeche / kreise[kreise$codeKreis == kreiscode, 5],
        wfl.1$Flaeche / kreise[kreise$codeKreis == kreiscode, 5]
      ), 1)
    ),
    Waldflaeche = data.frame(
      BWI = c(1987, 2002, 2012),
      ...
      SE_Waldfl_StW = round(c(
        wfl.stw.1$SE_Flaeche, wfl.stw.2$SE_Flaeche,
        wfl.stw.3$SE_Flaeche
      ), 0),
      Waldfl_KW = round(c(wfl.kw.1$Flaeche, wfl.kw.2$Flaeche, wfl.kw.3$Flaeche), 0),
      SE_Waldfl_KW = round(c(
        wfl.kw.1$SE_Flaeche, wfl.kw.2$SE_Flaeche,
        wfl.kw.3$SE_Flaeche
      ), 0),
      Waldfl_OEW = round(c(wfl.oew.1$Flaeche, wfl.oew.2$Flaeche, wfl.oew.3$Flaeche), 0),
      SE_Waldfl_OEW = round(c(
        wfl.oew.1$SE_Flaeche, wfl.oew.2$SE_Flaeche,
        wfl.oew.3$SE_Flaeche
      ), 0),
      Waldfl_PW = round(c(wfl.pw.1$Flaeche, wfl.pw.2$Flaeche, wfl.pw.3$Flaeche), 0),
      SE_Waldfl_PW = round(c(
        wfl.pw.1$SE_Flaeche, wfl.pw.2$SE_Flaeche,
        wfl.pw.3$SE_Flaeche
      ), 0),
      Waldfl_GPW = round(c(wfl.gpw.1$Flaeche, wfl.gpw.2$Flaeche, wfl.gpw.3$Flaeche), 0),
      SE_Waldfl_GPW = round(c(
        wfl.gpw.1$SE_Flaeche, wfl.gpw.2$SE_Flaeche,
        wfl.gpw.3$SE_Flaeche
      ), 0),
      Waldfl_MPW = round(c(wfl.mpw.1$Flaeche, wfl.mpw.2$Flaeche, wfl.mpw.3$Flaeche), 0),
      SE_Waldfl_MPW = round(c(
        wfl.mpw.1$SE_Flaeche, wfl.mpw.2$SE_Flaeche,
        wfl.mpw.3$SE_Flaeche
      ), 0),
      Waldfl_KPW = round(c(wfl.kpw.1$Flaeche, wfl.kpw.2$Flaeche, wfl.kpw.3$Flaeche), 0),
      SE_Waldfl_KPW = round(c(
        wfl.kpw.1$SE_Flaeche, wfl.kpw.2$SE_Flaeche,
        wfl.kpw.3$SE_Flaeche
      ), 0)
    ),
    Waldfl_Proz = data.frame(
      Eigentum = c("StW(BW)", "KW", "OEW", "PW", "GPW", "MPW", "KPW"),
      Waldfl_Proz = round(c(
        fl.ant.stw.3$Fl_Proz, fl.ant.kw.3$Fl_Proz,
        fl.ant.oew.3$Fl_Proz, fl.ant.pw.3$Fl_Proz, fl.ant.gpw.3$Fl_Proz,
        fl.ant.mpw.3$Fl_Proz, fl.ant.kpw.3$Fl_Proz
      ), 1),
      SE_Waldfl_Proz = round(c(
        fl.ant.stw.3$SE_Fl_Proz, fl.ant.kw.3$SE_Fl_Proz,
        fl.ant.oew.3$SE_Fl_Proz, fl.ant.pw.3$SE_Fl_Proz,
        fl.ant.gpw.3$SE_Fl_Proz,
        fl.ant.mpw.3$SE_Fl_Proz, fl.ant.kpw.3$SE_Fl_Proz
      ), 1)
    ),
    begehb_Holzbfl = data.frame(
      Eigentum =
        c("GW", "StW(BW)", "KW", "OEW", "PW", "GPW", "MPW", "KPW"),
      b_HBF = round(c(
        bhbf.3$Flaeche, bhbf.stw.3$Flaeche, bhbf.kw.3$Flaeche,
        bhbf.oew.3$Flaeche, bhbf.pw.3$Flaeche, bhbf.gpw.3$Flaeche,
        bhbf.mpw.3$Flaeche, bhbf.kpw.3$Flaeche
      ), 0),
      SE_begehb_Holzbfl = round(c(
        bhbf.3$SE_Flaeche, bhbf.stw.3$SE_Flaeche,
        bhbf.kw.3$SE_Flaeche, bhbf.oew.3$SE_Flaeche,
        bhbf.pw.3$SE_Flaeche, bhbf.gpw.3$SE_Flaeche,
        bhbf.mpw.3$SE_Flaeche, bhbf.kpw.3$SE_Flaeche
      ), 0)
    )
  ))
...
flaechen.stratum.fun.1 <- function(auswahl,
  ...
  fl.ant.stw.3 <-
    fl.proz.stratum.fun(
      auswahl.o, list(EigArt = c("BW", "StW")),
      ecken.3
    )
  # K\u00f6rperschaftswald
  fl.ant.kw.3 <-
