...
plot_group_stocks_by_girth <- function(b1, b2, b3, v,
    ...
    df_cast <-
        cbind(Durchmesser = factor(c(levels(df_cast$Durchmesser), "Alle"),
...
plot_group_area_by_age <- function(b1, b2, b3, a,
    ...
    df_cast <-
        cbind(Alter = factor(c(levels(df_cast$Alter), "Alle"),
...
plot_growth_loss <- function(g_l_12, g_l_23, g_l_12_all,
    ...
    names(df_cast) <- gsub("_Vorhersage", "", gsub(".*_Standardfehler", 
                                                           "Fehler",
                                                           names(df_cast)))
    label_row <- paste("& ", paste0("\\multicolumn{2}{c}{",
                                               levels(table_data$BWI), "}", 
                                               collapse =" & "), 
                       "\\\\"
                       )
...
d_name <- paste0(gsub(' ', '_', gsub('/', '_', file_name_district)))
    ...
    to_tex("\n\\begin{center}\\includegraphics[width=", 0.8, 
           "\\textwidth]{", out_path,
           "}\\end{center}")
    to_tex('Hier bezeichnet StW den Staatswald, KW den K\\"o{}rperschaftswald,',
...
if(TITLE_PLOT){ggplot2::ggtitle(title_district)}
    ...
    names(df_cast) <- gsub("period", "Periode", 
                           gsub(".*_prediction", 'Nutzung', 
                                gsub(".*_standard_error", "Fehler",
                                                           names(df_cast)
                                                           )
                                )
                           )
    label_row <- paste("Durchmesser & ", paste0("\\multicolumn{2}{c}{",
...
regeneration <- function(r2_a, r2_bs1, r2_bs2,
                         ...
                         file_name_district = regional_file_name,
                         title_district = krs.grupp$string[i]) {
    regeneration3 <- data.frame(group =  c(r3_a$BAGR, "Alle Arten"),
